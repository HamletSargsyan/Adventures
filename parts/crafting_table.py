...
class Craft:
    ...
    def craft_tool(self, item_name: str, progress_range: tuple, required_materials: dict):
        clear()
        if all(items[material]['Количество'] >= amount for material, amount in required_materials.items()):
    ...
    def craft_item(self, item_name: str, progress_range: tuple, required_materials: dict):
        clear()
        if all(items[material]['Количество'] >= amount for material, amount in required_materials.items()):
            alert(f"Вы создали {item_name.lower()}", 'success')
            items[item_name]['Количество'] += 1
